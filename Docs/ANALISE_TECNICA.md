...
> Blueprint de engenharia reversa do export `cables-export/js/Crowd.json` (344 ops, 479 conexões).
...
### Layout
A textura de 1590×360 tem **6 blocos de 60 frames**. Conteúdo identificado
visualmente no M0 do port (screenshots em `limiar-experience/shots/`):
**0 = idle · 1 = andar (default do patch) · 2 = idle variação · 3 = queda/morte
(one-shot) · 4 = levantar (one-shot) · 5 = rezar (loop)** — exatamente a máquina
de estados narrativa do projeto. O slider **"Posed Animations"
