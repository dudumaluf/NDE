...
1. **Pino**: teto de velocidade ×0,001 no agente seguido (update pass) +
   **walking forçado no state pass** (deixar a state machine ver v≈0
   ...
   patina). **2026-07-16:** stride/playback do pino são uniforms próprios
   (`pin stride/unit`, `pin playback ×`, `pin run playback ×` no leva) —
   independentes do `stride/unit` global da multidão. Heading do pino gira
   suave ao rumo do palco.
...
e escala `stageSpeed` pela distância (`steer speed ramp`, smoothstep até
`treadmill speed`); dentro da deadzone `stageSpeed→0` (rampa suave). O pino
promove walk/run conforme a fração do teto. No kill-switch (treadmill OFF) o
leme vira força direta (`steerDir` + `steerSpeedMul`: seek na direção + teto
proporcional à distância) que sobrepõe wander/gravidade só do seguido. Debug:
`steer wheel debug` desenha anéis no chão; sonda `__limiarSteerWheel`. Sonda:
o rumo responde ao ponteiro (dot cai), o mundo scrolla ~2 m em 2,5 s,
apontar no lado oposto
