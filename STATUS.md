...
Última atualização: **2026-07-15** (sessão UX follow/voz: auto-play no clique, overview+zoom suave, pause on hover, névoa na esteira, story field social, wander por grupo — **push para deploy online**)
...
| UX follow + voz | **Escada §4.1 fechada no código** (2026-07-15): (1) **auto-play** — clique na testemunha toca a 1ª estação com corte (sem 2º clique na timeline); (2) **overview 750 ms** — régua inteira visível antes do zoom; (3) **zoom ease-in-out** 1,2 s in / 0,8 s out (`timelineZoom.ts`); (4) **`pause on hover`** (Witnesses, default on) — para vida própria mas **permanece na esteira** (sem sliding); (5) **névoa scrolla** com `terrainU.scroll*` no follow (paridade chão/grid). Arquivos: `StoryTimeline.tsx`, `CrowdSim.ts`, `heightFog.ts`, `CrowdMesh.tsx`. Doc 04 §4.1/§4.2 | ✔ 2026-07-15 |
