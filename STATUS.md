...
Última atualização: **2026-07-13, madrugada** (controles de aparência + preset persistente do leva — os 5 pedidos diretos do Dudu; e M3.6 — estados de animação POR AGENTE dirigidos pela simulação; antes no mesmo dia: protótipo ONLINE na Vercel, frases do visitante, arco emocional + timeline normalizada nas 46 pessoas)
...
| Online (protótipo) | **https://limiar-prototipo.vercel.app** — Vercel Hobby (custo zero), projeto `limiar-prototipo`, deploy **prebuilt** (build local com content/ sincronizado + upload do `dist/` ~31 MB; conteúdo = HEAD `3f094cd`, manifest `c8c437af`). `noindex` + `X-Robots-Tag` (URL discreta, doc 00 §4). **Sem áudio** (2,7 GB ficam fora até o M4). Re-deploy: `npm run deploy` no `limiar-experience/` (seção Deploy do README) | ✔ 2026-07-12 |
